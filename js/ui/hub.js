import { $, esc, shuffle, toneBadge, toneCountBadge, ACT_NAMES, actTrackHTML } from '../engine/utils.js';
...
function milestoneRailHTML(G){
  ...
  return `<div class="panel tight ms-rail">
    ...
    <div class="ms-list">
      ${shown.map((m,i)=>`<div class="ms-row" role="button" tabindex="0" style="animation-delay:${i*0.05}s" onclick="viewChronicle(true)" onkeydown="if((event.key==='Enter'||event.key===' ')&&event.target===this){event.preventDefault();viewChronicle(true)}"><span class="ms-icon">${m.icon}</span><span class="ms-text">${esc(m.text)}</span></div>`).join('')}
    </div>
    ${all.length>shown.length?`<p class="small muted" role="button" tabindex="0" style="margin-top:6px;cursor:pointer" onclick="viewChronicle(true)" onkeydown="if((event.key==='Enter'||event.key===' ')&&event.target===this){event.preventDefault();viewChronicle(true)}">+${all.length-shown.length} earlier beat${all.length-shown.length===1?'':'s'} — open The Record</p>`:''}
  </div>`;
}
